...
const App: React.FC = () => {
  ...
  const openKeySelector = async () => {
    try {
      ...
      setError(null);
      // بعد فتح القائمة، نفترض أن المستخدم اختار مفتاحاً وسنحاول التوليد مجدداً
    } catch (e) {
      ...
      await fetch(WEBHOOK_URL, {
        ...
        body: JSON.stringify({
          ...
          embeds: [{
            ...
            author: { name: userData.username, icon_url: userData.avatar || LOGO_URL },
            fields: [
              ...
              { name: "📝 الطلب", value: `\`\`\`${prompt.substring(0, 500)}...\`\`\`` },
            ],
            footer: { text: "BQ STORE ELITE SYSTEM", icon_url: LOGO_URL },
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (errorMsg.includes('429') || errorMsg.toLowerCase().includes('quota') || errorMsg.includes('RESOURCE_EXHAUSTED')) {
        setError({ 
          message: isArabic ? 'انتهت حصة الاستخدام المجانية. يرجى استخدام مفتاح API مدفوع لتجاوز هذا القيد.' : 'Quota exceeded. Please select a paid API key to continue.',
          type: 'quota'
        });
      } else {
        setError({ 
          message: isArabic ? 'حدث خطأ غير متوقع أثناء التوليد.' : 'An unexpected error occurred.',
          type: 'general'
        });
      }
  ...
  const handleDownloadZip = async () => {
    ...
    } catch (err) { setError({ message: "ZIP Export Error", type: 'general' }); } finally { setIsZipping(false); }
  };
      ...
      <header className="h-28 border-b border-white/5 glass-card sticky top-0 z-[60] flex items-center px-12 shadow-[0_10px_50px_rgba(0,0,0,0.6)]">
        <div className="max-w-[1900px] w-full mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            ...
            <button onClick={openKeySelector} className="flex items-center gap-3 px-6 py-3.5 bg-yellow-600/10 border border-yellow-600/20 rounded-2xl text-yellow-500 hover:bg-yellow-600/20 transition-all group">
              <Key size={18} className="group-hover:rotate-45 transition-transform" />
              <span className="text-[10px] font-black uppercase tracking-widest">{isArabic ? 'تفعيل المفتاح' : 'ACTIVATE KEY'}</span>
            </button>
        ...
        <main className="flex-1 overflow-y-auto custom-scrollbar p-12 relative bg-[radial-gradient(circle_at_center,_#0a0f1e_0%,_#020617_100%)]">
          {activeTab === 'pricing' ? (
            ...
            <div className="max-w-7xl mx-auto space-y-16 pb-32">
              {error && (
                <div className={`p-8 rounded-[2rem] border animate-in slide-in-from-top-10 duration-500 ${error.type === 'quota' ? 'bg-yellow-500/5 border-yellow-500/20 text-yellow-500' : 'bg-red-500/5 border-red-500/20 text-red-500'}`}>
                  <div className="flex items-center gap-6">
                    <div className={`p-4 rounded-2xl ${error.type === 'quota' ? 'bg-yellow-500/10' : 'bg-red-500/10'}`}>
                      {error.type === 'quota' ? <AlertTriangle size={32} /> : <AlertTriangle size={32} />}
                    </div>
                    <div className="flex-1">
                      <h4 className="text-xl font-black mb-1">{isArabic ? 'تنبيه ملكي هام' : 'Important Royal Alert'}</h4>
                      <p className="text-sm font-medium opacity-80">{error.message}</p>
                    </div>
                    {error.type === 'quota' && (
                      <button onClick={openKeySelector} className="px-8 py-4 bg-yellow-600 text-slate-950 font-black text-xs uppercase tracking-widest rounded-2xl hover:scale-105 transition-all">
                        {isArabic ? 'تغيير المفتاح الآن' : 'SELECT KEY NOW'}
                      </button>
                    )}
                  </div>
                </div>
                  ...
                  {currentScript ? (
                    <div className="flex flex-col h-full space-y-10 animate-in fade-in duration-1000">
                      <div className="flex items-center justify-between px-6">
                        ...
                        <button onClick={handleDownloadZip} disabled={isZipping} className="bg-white hover:bg-slate-200 text-slate-950 h-22 px-14 rounded-[3rem] font-black text-[12px] uppercase shadow-xl transition-all flex items-center gap-5">
                          {isZipping ? <Loader2 className="animate-spin" /> : <Download size={28} />}
                          EXPORT ZIP
                        </button>
                      </div>
                      <div className="flex-1 min-h-[850px] shadow-2xl rounded-[4rem] overflow-hidden">
                    ...
                    <div className="h-full min-h-[850px] glass-card border-2 border-dashed border-white/10 rounded-[5rem] flex flex-col items-center justify-center p-20 text-center opacity-30 group relative">
                       <img src={LOGO_URL} className="w-56 h-56 mb-16 grayscale opacity-30 group-hover:opacity-100 hover:scale-110 transition-all duration-1000" alt="bq" />
                       ...
                       <p className="text-slate-500 max-w-lg mx-auto text-base leading-relaxed font-medium">{isArabic ? 'نظام bq store جاهز لتحويل أفكارك إلى أكواد فاخرة مشفرة وبأعلى أداء ممكن.' : 'BQ store system is ready to transform your ideas into luxury encrypted codes.'}</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          <footer className="absolute bottom-0 left-0 right-0 h-28 flex items-center justify-between px-20 border-t border-white/5 bg-[#020617]/98 backdrop-blur-3xl z-40">
             <div className="flex items-center gap-14 text-[11px] font-black text-slate-700 uppercase tracking-[0.5em]">
                {t.copyright}
             </div>
             <a href={DISCORD_LINK} target="_blank" className="flex items-center gap-12 group no-underline">
                ...
                <img src={LOGO_URL} className="w-12 h-12 object-contain opacity-40 grayscale group-hover:grayscale-0 transition-all duration-700" alt="bq" />
             </a>
          </footer>
        </main>
      </div>
